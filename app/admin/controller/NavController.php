    ...
    public function ajax_list() {
        ...
        if (!empty($navs)) {
            foreach ($navs as $key => $value) {
                $navs[$key]['is_main'] = $value['is_main'] ? "<i class='ion-checkmark i-icon icon-success add-tooltip' data-toggle='tooltip' data-original-title='是'></i>" : "<i class='ion-close i-icon icon-danger add-tooltip' data-toggle='tooltip' data-original-title='不是'></i>";
                $navs[$key]['edit_url'] = url('admin/nav/edit', array('id' => $value['id']));
                $navs[$key]['del_url'] = url('admin/nav/del', array('id' => $value['id']));
            }
            echo json_encode(['code' => 200, 'data' => $navs]);
            exit;
        }
